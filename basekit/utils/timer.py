...
def format_delta( start, end ):
    ...
    build_time_us = (
        delta.microseconds + delta.seconds * one_second + delta.days * one_day
    )
    ...
    if days > 0:
        ...
        if hours > 0:
            ...
            if seconds > 0:
                time_str = "%.2fs" % (
                    seconds + build_time_us / float(one_second)
                )
            else:
                ms = 0
                while build_time_us > one_millisecond:
                    ...
                    ms += 1
                time_str = "%.2fms" % (
                    ms + build_time_us / float(one_millisecond)
                )
    return time_str
...
class Timer:
    ...
    def __exit__(self, type, value, traceback):
        print "end '%s' in %s" % (
            self.name, format_delta( self.begin, now() )
        )
